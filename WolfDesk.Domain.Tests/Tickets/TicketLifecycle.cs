...
[FeatureFile("./Tickets/TicketLifecycle.feature")]
public sealed class TicketLifecycle : Feature
{
    private readonly Dictionary<string, UserId> _agents = new();
    private readonly Mock<IAgentsInformationService> _agentsInformationService = new();
    private readonly Dictionary<string, UserId> _customers = new();
    private readonly FakeClock _fakeClock = new();
    private readonly ProductId _product = new();

    private Ticket TheTicket { get; set; }
    ...
    public void InitTicket() {
        ...
        TheTicket = BuildTicket(aCustomer);
    }
    ...
    public void InitTicket(string priorityName, string categoryName) {
        ...
        TheTicket = BuildTicket(aCustomer, priority: priority, category: category);
    }
    ...
    public void SetSLAPolicy(string agentName, string priorityName, string categoryName, int hours) {
        ...
        _agentsInformationService.Setup(x => x.GetResponseDeadlineForAgent(agentId, priority, category))
            .Returns(TimeSpan.FromHours(hours));
    }
    ...
    public void SetCustomerResponseTime(string agentName, int days) {
        ...
        _agentsInformationService.Setup(x => x.GetResponseDeadlineForCustomer(agentId))
            .Returns(TimeSpan.FromDays(days));
    }
    ...
    private UserId FindUser(string name) {
        return _agents.ContainsKey(name) ? _agents[name] : _customers[name];
    }
}